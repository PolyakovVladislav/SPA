...
class SlotGame2ViewModel : ViewModel() {

    companion object {
        private val drawables = listOf(
            R.drawable.ic_slot_1_2,
            R.drawable.ic_slot_2_2,
            R.drawable.ic_slot_3_2,
            R.drawable.ic_slot_4_2,
            R.drawable.ic_slot_5_2
        )
    ...
    private fun getKoef(): Float {
        ...
        if (resultCombination.distinct().size == 1) {
            return 20f
        }

        val mostFreqElement = findMostFrequentValue(resultCombination)
        val frequency = resultCombination.count { it == mostFreqElement }
        if (frequency < 3 ) {
            return 0f
        } else if (frequency == 4 ) {
            return 5f
    ...
    private fun findMostFrequentValue(drawables: List<Int>): Int? {
        val frequencyMap = mutableMapOf<Int, Int>()

        for (number in drawables) {
            val count = if (frequencyMap.containsKey(number)) {
                frequencyMap[number]!!
            } else {
                0
            }
        ...
        var mostFrequentValue: Int? = null
        var maxFrequency = 0
